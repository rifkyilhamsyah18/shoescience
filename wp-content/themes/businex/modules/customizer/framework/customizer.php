...
if( ! class_exists( 'Businex_Customizer' ) ):

	class Businex_Customizer{
		...
		* 
		* @since  Businex 1.0.0
		* @uses   filter "Businex_Customizer_defaults"
		* @access public
		* @return array
		*/
	    public function get_defaults(){

	    	return apply_filters( 'Businex_Customizer_defaults', array() );
	    }
		...
		* @since  Businex 1.0.0
		* @uses   get_defaults()
		* @uses   filter "Businex_Customizer_fields"
		* @access public
		* @param  string $id
		* @return string | false
		*/
		public function get( $id ){
			...
			$fields   = apply_filters( 'Businex_Customizer_fields', array() );
		...
		*/
		public function register( $wp_customize ){
			...
			do_action( 'businex_customize_register', $wp_customize );

			$this->customize = $wp_customize;

			$this->fields   = apply_filters( 'Businex_Customizer_fields', array() );
			$this->panels   = apply_filters( 'Businex_Customizer_panels', $this->panels );
			$this->sections = apply_filters( 'Businex_Customizer_sections', $this->sections );

			$this->defaults = apply_filters( 'Businex_Customizer_defaults', $this->defaults );
			...
			foreach( $this->fields as $key => $field ){
				...
				switch ( $field[ 'type' ] ) {
					...
					case 'number':
						...
						if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
							$field[ 'sanitize_callback' ] = 'businex_sanitize_number';
						}
					...
					case 'checkbox':
						...
						if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
							$field[ 'sanitize_callback' ] = 'businex_sanitize_checkbox';
						}
					...
					case 'radio':
						...
						if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
							$field[ 'sanitize_callback' ] = 'businex_sanitize_choice';
						}
					...
					case 'dropdown-pages':
					if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
						$field[ 'sanitize_callback' ] = 'businex_sanitize_dropdown_pages';
					}
					break;
					
					case 'dropdown-posts':
					if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
						$field[ 'sanitize_callback' ] = 'businex_sanitize_dropdown_posts';
					}
					break;
					...
					if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
						$field[ 'sanitize_callback' ] = 'businex_sanitize_dropdown_categories';
					}
		...
		*/
		protected function add_panels(){
			...
			foreach ( $this->panels as $id => $panel ) {
				...
				$args = array(
				    'title' => empty( $panel[ 'title' ] ) ? esc_html__( 'No Title Specified.', 'businex' ) : $panel[ 'title' ],
				);
		...
		*/
		protected function add_sections(){
			...
			foreach ( $this->sections as $id => $section ){
				...
				$args = array(
					'title' => empty( $section[ 'title' ] ) ? esc_html__( 'No Title Specified.', 'businex' ) : $section[ 'title' ],
				);
		...
		protected function add_control( $field ){
			...
			switch( $field[ 'type' ] ){
				...
				case 'dropdown-categories':
					...
					$this->customize->add_control( new Businex_Customize_Category_Control( 
						$this->customize, 
						$control_id, 
						$args 
					) );
				...
				case 'dropdown-posts':
					...
					$this->customize->add_control( new Businex_Customize_Post_Control( 
						$this->customize, 
						$control_id, 
						$args 
					) );
...
* 
* @since  Businex 1.0.0
* @uses   Businex_Customizer::get_instance()->get( $id )
* @return string | bool
*/
if( !function_exists( 'businex_get_option' ) ){

	function businex_get_option( $id ){

		return Businex_Customizer::get_instance()->get( $id );
	}

}

/**
* Retrives default value of Businex_Customizer.
* 
* @since  Businex 1.0.0
* @uses   Businex_Customizer::get_instance()->get_defaults()
* @return object
*/
if( !function_exists( 'Businex_Customizer_get_defaults' ) ){

	function Businex_Customizer_get_defaults(){

		return Businex_Customizer::get_instance()->get_defaults();
	}

}

add_action( 'customize_register', array ( Businex_Customizer::get_instance(), 'register' ) );
