...
if( !function_exists( 'businex_enqueue' ) ):
/**
...
* @return void
*/
function businex_enqueue( $scripts ){

    # Do not enqueue anything if no array is supplied.
    if( ! is_array( $scripts ) ) return;

    $scripts = apply_filters( 'businex_scripts' , $scripts );
...
if( ! function_exists( 'businex_get_related_posts' ) ):
/**
* Get related posts
*
* @since Businex 1.0.0
...
* 
*/
function businex_get_related_posts( $taxonomy = array(), $per_page = 3, $get_params = false ){
...
* @return string	
*/	
function businex_modify_post_navigation_markup( $template, $class ){
...
add_filter( 'navigation_markup_template', 'businex_modify_post_navigation_markup', 10, 2 );

if( ! function_exists( 'businex_primary_menu_fb' ) ) :
/**
 ...
 * @return void
 */
function businex_primary_menu_fb( $arg ) {
    if( !$arg[ 'echo' ] ){
        ob_start();
    }
    ?>
    <ul>
        <li>
            <a href="<?php echo esc_url( home_url( '/' ) ) ?>"><?php esc_html_e( 'Home', 'businex' ) ?></a>
        </li>
    </ul>
    <?php
    if( !$arg[ 'echo' ] ){
...
if( !function_exists( 'businex_get_youtube_id' ) ):
/**
 ...
 * @return string | bool
 */
function businex_get_youtube_id( $url ) {
...
if( ! function_exists( 'businex_get_the_category' ) ):
/**
* Returns categories after sorting by term id descending
* 
* @since Businex 1.2
* @uses businex_sort_category()
* @return array
*/
function businex_get_the_category( $id = false ){
    $failed = true;
    ...
    if( $failed ){
      ...
      usort( $cat, 'businex_sort_category' );  
    }
...
if( ! function_exists( 'businex_sort_category' ) ):
/**
* Helper function for businex_get_the_category()
*
* @since Businex 1.0.0
*/
function businex_sort_category( $a, $b ){
    return $a->term_id < $b->term_id;
}
endif;

if( ! function_exists( 'businex_explode_string_to_int' ) ):
/**
...
*/
function businex_explode_string_to_int( $str ){
    if( empty( $str ) )
        return false;
...
if( ! function_exists( 'businex_get_menu' ) ):
/**
* Returns wp nav
*
* @since Businex 1.0.0
...
* @return String
*/
function businex_get_menu( $location = 'primary', $echo = true ){
    ...
    switch( $location ){
        ...
        case 'primary':
            $args[ 'fallback_cb' ] = 'businex_primary_menu_fb';
            $args[ 'menu_id' ] = 'primary-menu';
            $args[ 'menu_class' ] = 'primary-menu';
            $menu = wp_nav_menu( apply_filters( 'businex_'.$location.'_menu', $args ) );
        break;

        case 'topheader':
            $args[ 'fallback_cb' ] = 'businex_top_header_menu_fb';
...
if( !function_exists( 'businex_hex2rgba' ) ):
/**
* Convert hexdec color string to rgb(a) string
* @since Businex 1.0.0
*/
function businex_hex2rgba($color, $opacity = false) {
